function [] = quicklook_complex(frequency_resolution, fs)

r=audiorecorder(fs,16,2,1);

x = (-(fs/2):frequency_resolution:(fs/2-frequency_resolution));

FS=stoploop({'Click Stop','When Done'});

while(~FS.Stop())
    recordblocking(r,1/frequency_resolution);
    d = getaudiodata(r);
    d=j*d(:,1)+d(:,2); % sample bitfile has imaginary on left, real on right
    d=d-mean(d); % remove any DC offset from sound card
    subplot(2,1,1);
    plot(real(d)); 
    axis([0 length(d) -1 1]);
    hold on;
 ...
 hold off;  
    ...
    d= iqbalance(d); % this is to compensate for IQ imbalance as part of the recording system on your PC or the playback system on the ZED
    
    dwin=d.*hanning(length(d));
    f=fftshift((20*log10(abs(fft(dwin)))));
    %f=f(1:length(f)/2);
    plot(x,f);
    axis([-fs/2 fs/2 -50 100]);
    [maxval,ind]=max(f);
    hold on;
    tempstr=sprintf('%dHz\n%.1fdB',x(ind),f(ind));
    text(x(ind),maxval,tempstr);
    hold off;
    
end;

function b = iqbalance(d)
numpoints = length(d);
rrms = mean(abs(real(d)));
irms = mean(abs(imag(d)));
scalefact = irms/rrms;
b = real(d) .* scalefact + j*imag(d);
